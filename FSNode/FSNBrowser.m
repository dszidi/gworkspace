       ...
       selectionColumn:(BOOL)selcol
{
  ...
  if (self) {
    ...
    // NSInteger minCols = visibleColumns < 1 ? 2 : visibleColumns;
    NSInteger minCols = [columns count];
    for (i = 0; i < minCols; i++) {
    ...
    isLoaded = NO;	
	  // [self scrollColumnToVisible: lastVisibleColumn]; // TEST
...
- (void)tile
{
  ...
  if (updateViewsLock == 0) {
    ...
    // columnSize.width = myrintf(frameWidth / visibleColumns); // DEAD

    // columnSize.height = [_super_view bounds].size.height; // DEAD
    ...
    // [self updateScroller]; // What does this even do? Commenting it out has no visible effect
...
- (void)updateScroller
{
  if ((lastColumnLoaded == 0) || (lastColumnLoaded <= (visibleColumns - 1))) {
		...
		if (skipUpdateScroller == NO) {
			// The below values were based on dynamic column widths and are busted
			float proportion = (float)visibleColumns / (float)(lastColumnLoaded + 1);
        ...
        proportion = (float)visibleColumns / (float)(lastVisibleColumn + 1);
      }
      
			[scroller setFloatValue: f knobProportion: proportion];

			// [scroller setFloatValue: f knobProportion: visibleWidthProportion];
		}
...
- (void)resizeWithOldSuperviewSize:(NSSize)oldBoundsSize
{
  ...
  /*if (ncols != visibleColumns) {
    ...
    // TEST manually enabling horizontal scroll   
    // [scroller setEnabled: YES];
    // NSWindow *window = [self window];
    // [window enableFlushWindow];
    // [window flushWindowIfNeeded];
...
- (void)viewDidMoveToSuperview
{
  ...
  if ([self superview]) {
    ...
    [self tile];
	  [self scrollColumnToVisible: lastVisibleColumn];
  }
}
