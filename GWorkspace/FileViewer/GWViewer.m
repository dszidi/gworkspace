	  ...
	  withKey:(NSString *)key
{
  ...
  if (self) {
    ...
    if (defEntry && [defEntry floatValue] > MIN_SHELF_HEIGHT) {
...
- (void)createSubviews
{
  ...
  */
  ...
  hasScroller = YES;
  // hasScroller = ((viewType ==GWViewTypeIcon) || (viewType ==GWViewTypeList)); // Original configuration
  ...
  resizeMask = NSViewWidthSizable | NSViewHeightSizable; // <- Horizontal scroll doesn't work on browser
  ...
  if (tag > 0 && tag != viewType)
    {
      ...
      if (tag == GWViewTypeBrowser)
        {
          ...
          // [nviewScroll setDelegate: nil];
          [pathsView setOwnsScroller: NO];

          [nviewScroll setHasVerticalScroller: NO];
          [nviewScroll setHasHorizontalScroller: YES]; // Was NO

          nodeView = [[GWViewerBrowser alloc] initWithBaseNode: baseNode
                                                      inViewer: self
                                                visibleColumns: visibleCols
                                                      scroller: [nviewScroll horizontalScroller] //[pathsScroll horizontalScroller]
                                                    cellsIcons: NO
                                                 editableCells: NO
                                               selectionColumn: YES];
      ...
      NSRect r = [[vwrwin contentView] bounds];
      int cols = myrintf(r.size.width / [vwrwin resizeIncrements].width);  
      if (tag == GWViewTypeBrowser && cols != visibleCols) {
        //[self setSelectableNodesRange: NSMakeRange(0, cols)]; 
